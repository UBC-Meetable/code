(function () {
  ...
  diff_match_patch.prototype.diff_compute_ = function (a, b, c, d) {
    if (!a) return [[1, b]]; if (!b) return [[-1, a]]; let e = a.length > b.length ? a : b; let f = a.length > b.length ? b : a; let g = e.indexOf(f); return g != -1 ? (c = [[1, e.substring(0, g)], [0, f], [1, e.substring(g + f.length)]], a.length > b.length && (c[0][0] = c[2][0] = -1), c) : f.length == 1 ? [[-1, a], [1, b]] : (e = this.diff_halfMatch_(a, b)) ? (f = e[0], a = e[1], g = e[2], b = e[3], e = e[4], f = this.diff_main(f, g, c, d), c = this.diff_main(a, b, c, d), f.concat([[0, e]], c)) : c && a.length > 100 && b.length > 100 ? this.diff_lineMode_(a, b,
  ...
  diff_match_patch.prototype.diff_cleanupSemanticLossless = function (a) {
    function b(a, b) {
      ...
      if (a[c - 1][0] == 0 && a[c + 1][0] == 0) {
        var d = a[c - 1][1]; var e = a[c][1]; var f = a[c + 1][1]; var g = this.diff_commonSuffix(d, e); if (g) {
          var h = e.substring(e.length - g);
          var d = d.substring(0, d.length - g);
          var e = h + e.substring(0, e.length - g);
          var f = h + f;
        } for (var g = d, h = e, j = f, i = b(d, e) + b(e, f); e.charAt(0) === f.charAt(0);) { var d = d + e.charAt(0); var e = e.substring(1) + f.charAt(0); var f = f.substring(1); const k = b(d, e) + b(e, f); k >= i && (i = k, g = d, h = e, j = f); }a[c - 1][1] != g && (g ? a[c - 1][1] = g : (a.splice(c - 1, 1), c--), a[c][1] = h, j ? a[c + 1][1] = j : (a.splice(c + 1, 1), c--));
      }c++;
    }
    ...
    for (let b = this.Match_MaxBits, c = 0; c < a.length; c++) {
      if (!(a[c].length1 <= b)) {
        const d = a[c]; a.splice(c--, 1); for (var e = d.start1, f = d.start2, g = ""; d.diffs.length !== 0;) {
          const h = new diff_match_patch.patch_obj(); let
            ...
            var g = d.diffs[0][0]; var i = d.diffs[0][1]; g === 1 ? (h.length2 += i.length, f += i.length, h.diffs.push(d.diffs.shift()),
...
&& (h.length1 += i.length, h.length2 += i.length, h.diffs.length !== 0 && h.diffs[h.diffs.length - 1][0] === 0 ? h.diffs[h.diffs.length - 1][1] += i : h.diffs.push([0, i])); j || a.splice(++c, 0, h);
  ...
  diff_match_patch.prototype.patch_fromText = function (a) {
    const b = []; if (!a) return b; a = a.split("\n"); for (let c = 0, d = /^@@ -(\d+),?(\d*) \+(\d+),?(\d*) @@$/; c < a.length;) {
      let e = a[c].match(d); if (!e) throw Error(`Invalid patch string: ${a[c]}`); const f = new diff_match_patch.patch_obj(); b.push(f); f.start1 = parseInt(e[1], 10); e[2] === "" ? (f.start1--, f.length1 = 1) : e[2] == "0" ? f.length1 = 0 : (f.start1--, f.length1 = parseInt(e[2], 10)); f.start2 = parseInt(e[3], 10); e[4] === "" ? (f.start2--, f.length2 = 1) : e[4] == "0" ? f.length2 = 0 : (f.start2--, f.length2 = parseInt(e[4], 10)); for (c++; c < a.length;) { e = a[c].charAt(0); try { var g = decodeURI(a[c].substring(1)); } catch (h) { throw Error(`Illegal escape in patch_fromText: ${g}`); } if (e == "-")f.diffs.push([-1, g]); else if (e == "+")f.diffs.push([1, g]); else if (e == " ")f.diffs.push([0, g]); else if (e == "@") break; else if (e !== "") throw Error(`Invalid patch mode "${e}" in: ${g}`); c++; }
    } return b;
  }; diff_match_patch.patch_obj = function () { this.diffs = []; this.start2 = this.start1 = null; this.length2 = this.length1 = 0; };
  diff_match_patch.patch_obj.prototype.toString = function () { let a; let b; a = this.length1 === 0 ? `${this.start1},0` : this.length1 == 1 ? this.start1 + 1 : `${this.start1 + 1},${this.length1}`; b = this.length2 === 0 ? `${this.start2},0` : this.length2 == 1 ? this.start2 + 1 : `${this.start2 + 1},${this.length2}`; a = [`@@ -${a} +${b} @@\n`]; let c; for (b = 0; b < this.diffs.length; b++) { switch (this.diffs[b][0]) { case 1: c = "+"; break; case -1: c = "-"; break; case 0: c = " "; }a[b + 1] = `${c + encodeURI(this.diffs[b][1])}\n`; } return a.join("").replace(/%20/g, " "); };
