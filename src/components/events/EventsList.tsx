...
const EventsList = ({ navigation, courseId }:EventsListProps) => {
  const { events, loading } = useEvents();
  let flattenedEvents: Event[] = [];
  if (courseId) {
    flattenedEvents = events.get(courseId) || [];
  } else {
    flattenedEvents = useMemo(() => eventMapToFlatArray(events), [events]);
  }
  ...
  const renderItem = ({ item }: { item: Event }) => {
    ...
    return (
      <EventBubble
        ...
        title={item.title}
        courseName={courseId ? "" : groupTitle}
      />
    );
  };
