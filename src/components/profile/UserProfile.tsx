...
const UserProfile = ({ user, style }) => {
    const { firstName, lastName, email, bio, profilePicture, interests = [] } = user;
    ...
    return (
        <ScrollView contentContainerStyle={{
            ...
            <Layout style={{ display: "flex", backgroundColor: "white", padding: 10, width: "100%", marginVertical: 10, borderRadius: 10, alignItems: "center", justifyContent: "center" }}>
            ...
            <Layout style={{ padding: 10, borderRadius: 20, width: "100%", display: "flex", flexDirection: "row", flexWrap: "wrap", alignItems: "center", justifyContent: "center" }}>
                {interests.length ? interests.map((interest, index) => <Chip key={index} style={{ margin: 5, backgroundColor: "#FEEDDE" }} textStyle={{ fontFamily: "Poppins_500Medium", fontSize: 12 }}>{interest}</Chip>)
